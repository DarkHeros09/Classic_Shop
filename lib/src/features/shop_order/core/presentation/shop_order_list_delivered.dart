...
final shopOrderCardDeliveredIndexProvider =
    Provider<int>((_) => throw UnimplementedError());

class ShopOrderListDelivered extends StatefulHookConsumerWidget {
  const ShopOrderListDelivered({super.key});
  @override
  ConsumerState<ShopOrderListDelivered> createState() =>
      ShopOrderListDeliveredState();
}

class ShopOrderListDeliveredState
    extends ConsumerState<ShopOrderListDelivered> {
  @override
  Widget build(BuildContext context) {
    final state = ref.watch(shopOrdersDeliveredNotifierProvider);
    ...
    return SuperSliverListSeparated(
      ...
      itemBuilder: (context, index) => ProviderScope(
        key: UniqueKey(),
        overrides: [
          shopOrderCardDeliveredIndexProvider.overrideWithValue(index),
        ],
        child: state.map(
...
class OrderDetailsAndStatus extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final appTheme = Theme.of(context);
    final isDarkMode = appTheme.brightness == Brightness.dark;
    final index = ref.watch(shopOrderCardDeliveredIndexProvider);
