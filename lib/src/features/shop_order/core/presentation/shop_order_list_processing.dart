...
final shopOrderCardProcessingIndexProvider =
    Provider<int>((_) => throw UnimplementedError());

class ShopOrderListProcessing extends StatefulHookConsumerWidget {
  const ShopOrderListProcessing({super.key});
  @override
  ConsumerState<ShopOrderListProcessing> createState() =>
      ShopOrderListProcessingState();
}

class ShopOrderListProcessingState
    extends ConsumerState<ShopOrderListProcessing> {
  @override
  Widget build(BuildContext context) {
    final state = ref.watch(shopOrdersProcessingNotifierProvider);
    ...
    debugPrint('_itemCountProcessing $itemCount');
    return SuperSliverListSeparated(
      itemCount: itemCount,
      itemBuilder: (context, index) => ProviderScope(
        key: UniqueKey(),
        overrides: [
          shopOrderCardProcessingIndexProvider.overrideWithValue(index),
        ],
        child: state.map(
...
class OrderDetailsAndStatus extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final appTheme = Theme.of(context);
    final isDarkMode = appTheme.brightness == Brightness.dark;
    final index = ref.watch(shopOrderCardProcessingIndexProvider);
